...
namespace GameCreator.Runtime.Perception.UnityUI
{
    ...
    public class IndicatorAwarenessItemUI : MonoBehaviour
    {
        ...
        [SerializeField]
        private CanvasGroup m_Opacity;

        [SerializeField]
        private GameObject m_ActiveIfNotZero;

        [SerializeField]
        private GameObject m_ActiveIfSuspicious;

        [SerializeField]
        private GameObject m_ActiveIfAlert;

        [SerializeField]
        private GameObject m_ActiveIfAware;

        [SerializeField]
        private GameObject m_ActiveIfOnscreen;

        [SerializeField]
        private GameObject m_ActiveIfOffscreen;

        [SerializeField]
        private RectTransform m_RotateTo;

        [SerializeField]
        private ProgressAwareness m_Awareness = new ProgressAwareness();

        // MEMBERS: -------------------------------------------------------------------------------

        [NonSerialized]
        private float m_PreviousAwareness = 0;
        ...
        public void Refresh(float awareness, Color color, bool onScreen, float rotation)
        {
            ...
            if (this.m_Opacity != null)
                this.m_Opacity.alpha = awareness;
            ...
            {
                this.m_ActiveIfNotZero.SetActive(
                    this.m_ActiveIfNotZero.activeSelf
            ...
            if (this.m_ActiveIfSuspicious != null)
                this.m_ActiveIfSuspicious.SetActive(stage == AwareStage.Suspicious);
            if (this.m_ActiveIfAlert != null)
                this.m_ActiveIfAlert.SetActive(stage == AwareStage.Alert);
            if (this.m_ActiveIfAware != null)
                this.m_ActiveIfAware.SetActive(stage == AwareStage.Aware);

            if (this.m_ActiveIfOnscreen != null)
                this.m_ActiveIfOnscreen.SetActive(onScreen);
            if (this.m_ActiveIfOffscreen != null)
                this.m_ActiveIfOffscreen.SetActive(!onScreen);
