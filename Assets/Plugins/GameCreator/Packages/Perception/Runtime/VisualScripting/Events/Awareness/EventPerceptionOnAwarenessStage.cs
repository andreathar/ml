...
namespace GameCreator.Runtime.Perception
{
    ...
    public class EventPerceptionOnAwarenessStage : VisualScripting.Event
    {
        ...
        [SerializeField]
        private AwareMask m_Stage = AwareMask.Suspicious;

        [SerializeField]
        private When m_When = When.OnIncrease | When.OnDecrease;
        ...
        private void OnChangeAwarenessStage(GameObject target, AwareStage stage)
        {
            ...
            if (!this.m_Target.Match(target, args))
                return;

            AwareStage previousStage = this.m_PreviousStages.GetValueOrDefault(
                target,
                AwareStage.None
            );
            this.m_PreviousStages[target] = stage;

            if (!this.m_Stage.HasFlag((AwareMask)stage))
                return;
            if (!this.m_When.HasFlag(When.OnIncrease) && previousStage < stage)
                return;
            if (!this.m_When.HasFlag(When.OnDecrease) && previousStage > stage)
                return;
