...
namespace GameCreator.Runtime.Perception
{
    ...
    public class Luminance : MonoBehaviour
    {
        ...
        [SerializeField]
        private PropertyGetDecimal m_Multiplier = GetDecimalConstantOne.Create;
        ...
        public float IntensityTo(Transform target)
        {
            if (target == null)
                return 0f;
            if (this.Light == null || !this.Light.isActiveAndEnabled)
                return 0f;
        ...
        private float GetSpotIntensityTo(Transform target)
        {
            ...
            if (angle > spotOuterAngle || spotOuterAngle <= float.Epsilon)
                return 0f;
            float angleRatio =
                1f - (angle > spotInnerAngle ? (angle - spotInnerAngle) / spotOuterAngle : 0f);
            ...
            Vector3 projection = TransformUtils
                ...
                .PointOnVector(Vector3.forward);

            float projectionMagnitude = projection.magnitude;
            if (projectionMagnitude > this.Light.range)
                return 0f;
