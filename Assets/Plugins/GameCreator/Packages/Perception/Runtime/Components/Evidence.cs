...
namespace GameCreator.Runtime.Perception
{
    ...
    public class Evidence : MonoBehaviour, ISpatialHash
    {
        ...
        [SerializeField]
        private PropertyGetString m_Tag = GetStringId.Create("my-evidence-tag");

        [SerializeField]
        private bool m_OnSight = true;

        [SerializeField]
        private bool m_EmitNoise;

        [SerializeField]
        private PropertyGetDecimal m_NoiseRadius = GetDecimalDecimal.Create(5f);

        [SerializeField]
        private PropertyGetDecimal m_NoiseLevel = GetDecimalConstantOne.Create;

        [SerializeField]
        private bool m_EmitScent;

        [SerializeField]
        private PropertyGetDecimal m_ScentInterval = GetDecimalConstantOne.Create;

        [SerializeField]
        private PropertyGetDecimal m_ScentRadius = GetDecimalDecimal.Create(3f);

        [SerializeField]
        private PropertyGetDecimal m_ScentLevel = GetDecimalConstantOne.Create;

        [SerializeField]
        private bool m_StartTampered;
        ...
        [NonSerialized]
        private bool m_IsTampered;

        [NonSerialized]
        private float m_LastTimeEmitScent;
        ...
        public bool IsTampered
        {
            ...
            set
            {
                if (this.m_IsTampered == value)
                    return;
                this.m_IsTampered = value;

                if (this.m_IsTampered)
                    this.EventTamper?.Invoke();
        ...
        private void UpdateScent()
        {
            ...
            if (Time.time < this.m_LastTimeEmitScent + interval)
                return;
