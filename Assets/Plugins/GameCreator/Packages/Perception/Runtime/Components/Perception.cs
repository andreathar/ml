...
namespace GameCreator.Runtime.Perception
{
    ...
    public class Perception : MonoBehaviour, ISpatialHash
    {
        ...
        [SerializeField]
        private bool m_CanForget;

        [SerializeField]
        private PropertyGetDecimal m_Duration = new PropertyGetDecimal(30f);

        [SerializeField]
        private PropertyGetDecimal m_ForgetSpeed = GetDecimalConstantPointFive.Create;

        [SerializeField]
        private PropertyGetDecimal m_ForgetDelay = GetDecimalConstantOne.Create;

        [SerializeField]
        private SensorList m_Sensors = new SensorList();

        // MEMBERS: -------------------------------------------------------------------------------

        [NonSerialized]
        private Cortex m_Cortex = new Cortex();

        // PROPERTIES: ----------------------------------------------------------------------------

        internal float AwareDuration =>
            this.m_CanForget ? (float)this.m_Duration.Get(this.Args) : float.MaxValue;
        ...
        [field: NonSerialized]
        internal Args Args { get; private set; }
        ...
        [field: NonSerialized]
        public GameObject LastNoticedEvidence { get; private set; }
        ...
        public void RelayAwareness(Perception other)
        {
            ...
            foreach (KeyValuePair<int, Tracker> entry in this.m_Cortex.TrackerList)
            {
                ...
                if (otherTracker == null)
                    continue;
                if (otherTracker.Awareness > entry.Value.Awareness)
                    continue;
        ...
        public void RelayEvidence(Perception other)
        {
            ...
            foreach (KeyValuePair<string, bool> entry in this.m_Cortex.Evidences)
            {
                ...
                newEvidences =
                    newEvidences
                    || !other.m_Cortex.Evidences.TryGetValue(entry.Key, out bool value)
                    || !value;
