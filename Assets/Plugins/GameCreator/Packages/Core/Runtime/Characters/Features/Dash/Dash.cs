...
namespace GameCreator.Runtime.Characters
{
    ...
    public class Dash
    {
        ...
        public bool CanDash()
        {
            if (this.m_Character.Busy.AreLegsBusy)
                return false;

            bool canDashInAir = this.m_Character.Motion.DashInAir;
            if (!canDashInAir && !this.m_Character.Driver.IsGrounded)
                return false;
            ...
            if (this.m_Character.Time.Time >= resetTime)
                return true;
        ...
        )
        {
            ...
            float resetTime = this.m_LastDashFinishTime + this.m_Character.Motion.DashCooldown;
            this.m_NumDashes = this.m_Character.Time.Time < resetTime ? this.m_NumDashes + 1 : 1;
            ...
            direction =
                direction.sqrMagnitude > float.Epsilon ? direction.normalized : Vector3.forward;
            ...
            while (this.IsDashing && !ApplicationManager.IsExiting)
                await Task.Yield();
